...
This project aims at building a robust route planner that will compute the fastest route between departure and arrival stops within a provided confidence tolerance expressed as interquartiles. Different means of transport are considered over all days of the week and all datas are taken from the SBB dataset (see description in the next [sections](https://dslab-gitlab.epfl.ch/squadraelectrica/final-project/-/blob/master/README.md#dataset-description)).

At first we imported the data using hadoop: after running _kinit_ _<username>_ it is possible to build the different tables in hive executing the corresponding notebook [1](https://dslab-gitlab.epfl.ch/squadraelectrica/final-project/-/blob/master/notebooks/1_HDFS_hive_env.ipynb) and [2](https://dslab-gitlab.epfl.ch/squadraelectrica/final-project/-/blob/master/notebooks/2_Data_prep.py). The goal is to build tables containing only the necessary information to run the Connection Scan Algorithm (CSA) that is used to predict the route.

Once the tables have been created, the datas can be processed to consider only the stations in the Zürich area (within a certain range R=15 km) and to build the tables with a selected time schedule. This selection is necessary to simplify the execution of the algortihm. Moreover, running the same [notebook](https://dslab-gitlab.epfl.ch/squadraelectrica/final-project/-/blob/master/notebooks/3_Spark_processing.py), the delays information are pre-processed, so that they can be exploited to find the route with the lowest probable delay. 

The CSA algorithm used to solve the problem can be found [here](https://dslab-gitlab.epfl.ch/squadraelectrica/final-project/-/blob/master/CSA_algorithm.pdf), and the implementation of the CSA algorithm can be found [here](https://dslab-gitlab.epfl.ch/squadraelectrica/final-project/-/blob/master/notebooks/4_CSA.py). The function receives as input the starting stop, the arrival one, the arrival time, a weekday table, a table of footpaths and the confidence. At the output the route with the latest departure time is obtained, according to the given confidence.  

Finally, the user can visualize the obtained output at the end of this [notebook](https://dslab-gitlab.epfl.ch/squadraelectrica/final-project/-/blob/master/notebooks/5_Visualisation.ipynb). He can select the starting stop, the end stop, the desired arrival time and the day of the trip. The code will show him a map of his trip, highlighting the departure and arrival stations. 
    
In order to validate our results, we have added tests cases comparing our algorithm with Google Maps results. Also, we have looked into how the risk of delays influences the algorithm and how it diverges from standard planners such as SBB or Google Maps. You may have a look at our validation part at the end of [notebook 5](https://dslab-gitlab.epfl.ch/squadraelectrica/final-project/-/blob/master/notebooks/5_Visualisation.ipynb)

For further details, [here]() there is a reference video. 
